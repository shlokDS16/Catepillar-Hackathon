# Track B task list (to review 1): revision 3 (G2 re-check 2 applied)
...
1. H0 on `main`: one commit installs every known dependency for both tracks (root and workspace
   `package.json`, `pnpm-lock.yaml`), before the worktrees are created. Track B's list is in §6; Track F's
   list is Track F's (G2 names next-intl, maplibre-gl, @supabase/supabase-js, vitest). Each worktree then
   runs `pnpm install` (node_modules is not shared).
2. Each track logs in `docs/sessions/track-b.md` / `track-f.md`; only the orchestrator edits
   `CHECKLIST.md` and `STATE.md`.
3. **One integrator** (G2 #2): the orchestrator session in the primary folder, which holds `main`. At each
   integration point it runs `git merge track-f` then `git merge track-b`, then `vercel deploy --prod` from
   that folder (the `.vercel/` link lives there); both tracks then run `git merge main`. Tracks announce a
   merge-ready commit in their log first. No new dependency after H0 except through the integrator on
   `main`.
...
- psql is not installed; SQL **unit** tests use `scripts/sqltest.ts` (Node `postgres` driver) through the
  session pooler, each file wrapped in `begin; … rollback;`. They call the **step functions**
  (`private.step_*`, `escalate_once`), never the procedures: a procedure that COMMITs cannot run inside a
  transaction block (R2-1).
- **Integration tests** for the procedures (`worker_step`, `escalate_step`) run **outside** a transaction
  on a **local stack** (`supabase start`, then `supabase db reset` before each suite; Docker 29.3.1 is
  installed and must be running). Fallback if the local stack is not usable: run them against the shared
  project only at an integration point, in a `rehearsal` run, then `purge_run` (§0.6 rule respected).
...
|---|---|---|---|
| **B0** (1 h) | Link the project; `config.toml` with `verify_jwt = false` for **every** function (auth in code, G2-6); Edge Function secrets from `.env` + `PUBLIC_FUNCTIONS_URL` + `LLM_CHAIN` + `GROQ_API_KEY`, `GOOGLE_GENERATIVE_AI_API_KEY`, `GROQ_API_KEY_BACKUP` (D9); secret key, supervisor chat id and site emergency tel into Vault; Realtime "Allow public access" off; uv PATH; sqltest skeleton; **provider smoke test** (Telegram sendMessage, one Twilio call to a verified phone, Groq A and B chat, prompt guard, safeguard, Gemini text, Pinecone, Voyage); check org usage (paused projects vs 500 MB, G2-15); read `cron.use_background_workers` (N5); check whether `cron.schedule_in_database(…, username)` and role-level `statement_timeout` are allowed (N4); `cron.log_statement` (G2-21); **COMMIT probe (R2-1)**: create `private.probe()` (SECURITY INVOKER, no `SET`: insert A, `commit`, insert B, raise), schedule `'1 seconds'` → `call private.probe();`, expect A present, B absent, `job_run_details.status = failed`, unschedule; if it fails, switch the worker design to one job per step (DM §6 fallback) before B9 | image version ≥ 15.1.1.61 from the Management API (`database.version`), **and** a probe `'1 seconds'` job runs then is unscheduled; smoke test green; findings in the track log | sqltest smoke; smoke script output |
| **B0b** (15 m) | **Web deploy (P0)**, run by the integrator from the primary folder: `vercel link` + first `vercel deploy --prod` of `apps/web` from `main`; Vercel env: the `NEXT_PUBLIC_*` values and **`DEMO_DRIVER_SECRET` as a server-only variable** (never `NEXT_PUBLIC_`; UI-21); HTTPS URL tested on the Android demo phone. Fallback if `vercel login` fails at H0: connect the Vercel project to the GitHub repo in the dashboard so a push to `main` deploys (G2) | the phone loads the HTTPS URL and vibrates on the test button | manual check in the track log |
...
| **B5** (1 h) | Migration 003: ledger: `ledger_queue`, drain step, canonical v1, `ledger_write_one` validator, `ledger_verify` (internal), `ledger_recompute(first,last)`, Merkle over **raw 32-byte** hashes, `ledger_roots`, `ledger_checkpoint`, `demo_tamper` (rehash also rewrites `root_hex` **and** `head_hash`); **`ledger_export(first,last)`** (no lock) and the **browser TS verifier** `packages/shared/src/ledger/verify.ts` (canonical v1 + WebCrypto SHA-256 + chain + Merkle; R2-2) with `scripts/verify-ledger.ts` running the same code offline; `ledger_verify` with a 10 s lock timeout → `ledger_busy` (R2-5); **every mutator and the verify snapshot take lock 4210001** (N3) | golden vectors 1-3 byte for byte in SQL and TS; the TS verifier on exported rows reproduces vector 3's root and, after `rehash` **plus a redefined `ledger_recompute`**, still reports a mismatch against the published line (reviewer's R2-2 repro); edit → hash_mismatch; rehash → internal ✓ and recomputed root ≠ the sent line; a checkpoint racing the writer is never self-inconsistent | sqltest (incl. concurrent checkpoint + drain) + vitest vectors |
| **B9** (1 h) | Scenario engine inside the **`worker` procedure** (steps with `COMMIT` between them: ledger drain, Loop queue, tick, every-5th-second requeue/housekeeping; each step in its own exception block); tick: try-lock 4210002 (also used by `manual_tick`), ≤ 200 frames, per-frame exception → `tick_errors`, catch-up scaled by speed, progress from load cycles, `motion_locked`/`nearest` on `operator_state`, `clock` (with `server_now`) + delta `machines`, `tick_log`, `purge_run` | 60× plays 8 h in 8 min with zero `catch_up` suppressions; a poison frame is logged and skipped; **timeout probe** (a step sleeping 10 s is cancelled by the role backstop, or the gap is logged; N4) | sqltest: due frames, double tick no-op, catch-up at 1×/10×/60×, poison frame, probe (step functions in rollback); **integration suite** on the local stack after `db reset`: `call private.worker_step()` commits per step, poison frame run (R2-1) |
| **B10** (45 m) | Pure rule detectors (P0 six rules) + `apply_findings` (anomalies with location, **`explanation {en, hi}` rendered by named-slot `replace()` from `EXPLANATION_TEMPLATES`, `fuel_l`, `cost_inr` by the api-contracts §12 formula, `severity_tier`**; events; alerts; ledger only via the event trigger) | demo seatbelt frame → exactly one event, alert, queue entry, loop_queue row; the idle-excess fixture yields the expected EN and HI sentence and cost (e.g. 14 L, ≈ ₹1,290 at ₹92/L) | sqltest: positive + negative per rule; explanation/cost golden rows; `sd = 0` and nulls never raise |
| **B10b** (1 h) | Loop builder **as worker step 2** (N2): drain `loop_queue` ≤ 20; lesson assignment; `build_replay` for `safety.seatbelt_breach` from the event record + scenario frames into the **four-phase** `ReplayScenario` (D10: brief; seven evidence card types with `relevant` kept server-side; sequenced decide steps with countdowns and protocol order; re-enactment track of trail, machine and wind); `replay_get`; `replay_submit` → safety/procedure/efficiency scores, process trace vs ideal, rule quoted from the fixed protocol card | seatbelt event → one assignment + one replay that parses with `ReplayScenario`; no answer key in the client JSON; **a raising `build_replay` leaves the event, alert and ledger entry intact** (N2 repro) | sqltest + vitest (parse stored JSON; scoring fixtures); the raising-`build_replay` repro runs in the **integration suite** (outside a transaction) |
...
## 2. Subagent lane (parallel, `scripts/` + `supabase/functions/ask/` + `supabase/functions/demo-login/`; B23 writes generated audio for the integrator, ≈ 7 h of subagent time)
...
| **B16** (1 h) | ETA: `scripts/eta-fit.ts` → `packages/shared/src/eta/model.v1.json` (handed to the main lane for commit); evidence `eta.*`; p50/p90 + factors seeded for scenario tasks; **model P50 for the `test` split written so `v_task_analytics` returns `TaskAnalyticsRow`s** (the one P0 supervisor chart) | MAE vs organiser estimate reported honestly; P90 coverage 0.85-0.95; the view returns one row per task_type × condition with n ≥ 5 | vitest; sqltest on the view |
...
| **B23** (1 h) | **Hindi/English audio (D7 Sarvam; UI-17)**: `packages/shared/src/audio/phrases.ts` registry (alert tiers per kind, Guardian protocol lines, SOS states, lesson card lines + quiz prompts, replay brief/step/rule lines; the EN and HI texts incl. the explanation templates, reviewed by a native Hindi reader) → `scripts/tts/generate.ts`: Sarvam `POST /text-to-speech`, `bulbul:v3`, `language_code` hi-IN / en-IN, `output_audio_codec: mp3` [V docs], keys `SARVAM_API_KEY_1` then `_2`, only phrases whose `text_sha256` changed → `apps/web/public/audio/{lang}/{phrase_id}.mp3` + `packages/shared/src/audio/manifest.json`. Gemini TTS (`gemini-2.5-flash-preview-tts`) only if Sarvam fails, flagged unverified for Hindi. Files are committed by the integrator (the folder is in Track F's tree) | every phrase has `ok` or `missing` in the manifest; all P0 alert and replay phrases `ok` by ≈ H11; re-run is a no-op | vitest: registry ids unique, manifest schema, no phrase > 2,500 chars |
| **B24** (2 × 45 m: **B24a** OSHA/NIOSH curation + fetch; **B24b** SOPs + DGMS summaries) | **RAG corpus** in `scripts/corpus/` with `manifest.json` (url or author, licence, audience, language, sha256): (a) **≈ 25 OSHA/NIOSH public-domain PDFs**. Research 16 does not list 25 exact PDF URLs; it gives the sources and licence basis: `osha.gov/publications` (catalogue), OSHA public-domain interpretation `osha.gov/laws-regs/standardinterpretations/1992-09-15`, NIOSH reuse policy `cdc.gov/other/agencymaterials.html`, NIOSH `cdc.gov/niosh/publications/numbered/2000-120.html` and Pubs 2000-128 and 2007-146 (haul-truck collision warning, proximity warning). Step 1 therefore curates the 25 URLs from those catalogues, fetches each (HTTP 200, PDF, OSHA/NIOSH host) and records it; no Cat O&M text, no ISO clause text [R research 16 §5]. (b) **≈ 20 team-authored SOPs** written during the build (site-specific, EN, with a HI summary; seatbelt on slopes, faulty machine nearby, hydraulic leak, fire, PPE, SOS, heat, cold, reversing, refuelling …). (c) **≈ 8 DGMS summaries**, paraphrased and cited, never bulk-copied [R]. Ingestion by B18: LlamaParse → Voyage → Pinecone `spotter-kb` | manifest has ≥ 50 entries, each with a licence line; every URL verified at fetch time | a manifest validator script |
| **B24c** (20 min, **Shlok + Aryan**) | **SOP review gate (R2-3)**: `scripts/corpus/review.md` lists every team SOP and DGMS summary with a one-line summary and a link; they skim and tick approve/reject; `scripts/corpus/approve.ts` writes `review_status = 'reviewed'`, `reviewed_by`, `reviewed_at` into the manifest; B18 re-ingests metadata. OSHA/NIOSH are auto-'reviewed' | every SOP is 'reviewed' or 'draft' with a reviewer name; the demo's safety questions have at least one reviewed chunk | manifest validator |
...
| RAG corpus in `scripts/corpus/` (B24a/B24b), SOP approval by Shlok + Aryan (B24c) | B18 (≈ H3); B24c before the dry run |
| `GOOGLE_GENERATIVE_AI_API_KEY` (not in `.env`) | B19 |
| `vercel login` fixed | **H0** (blocks B0b; deploy is P0) |
| TTS: **Sarvam** (D7; `SARVAM_API_KEY_1`, `_2` in `.env`); Gemini TTS only if Sarvam fails (Hindi unverified) | B23, clips by ≈ H11 |
...
## 6. Dependency graph (execute in this order) and the H0 dependency list

A task starts only when every task on its left has merged into the track branch (G2 #3 order).
```
H0 (integrator: dependency commit, worktrees, pnpm install, .env + apps/web/.env.local, docs/sessions/)
 └─ B0 ─┬─ B0b (web deploy)                                  ─ needs Shlok's vercel login or the GitHub fallback
        ├─ B1 (contracts freeze = IP0) ─┬─ B2 ─ B4 ─ B5
        │                               ├─ B6 → B7 → B8        B8 needs B2 + B4 (schema) and B6/B7 (data)
        │                               ├─ B22                 (needs B1 DemoLogin schema)
        │                               ├─ B23                 (needs B1 phrase/audio schemas; lesson + replay texts from B8's content)
        │                               └─ B24a + B24b → B24c (humans) → B18 → B19   (B19 also needs B4, B15 for live tools)
B4 + B5 + B8 ─ B9 ─ B10 ─ B10b ─ B11 ─ B12 ─ B15 ─ B13 ─ B14 ─ B21
   (B9 may be developed on a 20-frame fixture, but its acceptance run needs B8's `review1` scenario)
B7 + B2 ─ B16 (needs B1 ETA schema; fills v_task_analytics)
B10 + B11 + B7 + B8 ─ B17 (evaluation sandbox; needs the pure detectors and the labels)
```
Main lane order: **B0, B0b, B1, B2, B4, B5, B9, B10, B10b, B11, B12, B15, B13, B14, B21**.
Subagent lane order: **B6 → B7 → B8 (by IP1), B24a, B24b → B24c (Shlok + Aryan, 20 min) → B18 → B19, B16, B17, B22, B23** (B23 by ≈ H11).

**H0 dependency commit, Track B's part** (package names only; versions are pinned by the lockfile of that
commit, not guessed here):
| Where | Packages | Used by |
|---|---|---|
| root devDependencies | `vitest`, `tsx` (run TS scripts) | all TS tests and `scripts/*.ts` |
| `packages/shared` | `zod` (already `^4.6.5`) | contracts |
| `scripts/` (workspace package `scripts`) | `postgres` (sqltest, seed, eval), `@supabase/supabase-js` (seed: admin users); LlamaParse, Voyage, Pinecone, Sarvam and Open-Meteo are called with `fetch` (no SDK) | B0, B8, B17, B18, B23, B24 |
| `scripts/generator` (uv project, Python 3.14) | `numpy`, `pytest` (weather via stdlib `urllib`) | B6, B7 |
| Edge Functions | none: Deno `fetch` adapters; `npm:@supabase/supabase-js` imported by specifier at deploy | B13-B15, B19, B22 |
Also at H0: add `CONNECTION_STRING` (session pooler) to `.env` (G2), used by `scripts/sqltest.ts`.
